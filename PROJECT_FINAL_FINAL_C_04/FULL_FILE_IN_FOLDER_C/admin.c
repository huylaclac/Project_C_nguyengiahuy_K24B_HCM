...
void addUser() {
    ...
    int isDuplicate = 1;
    for(int i = 0; i < userCount; i++) {
        if(strcmp(users[i].id, newUser.id) == 0 ||             // ham strcmp so sanh 2 chui, neu 2 chui = nhau tra ve khong, so amm neu 1<2, duong neu 1>2 
...
void searchUserByName() {
    ...
    for(int i = 0; i < userCount; i++) {                        
		...
		if(!found){
        printf("\033[31m User with NAME '%s' not found. \033[0m\n", searchName);
    }
}
//.
...
void sortUserAscending() {
	...
	for( int i =0;i< userCount-1;i++){                    // vong lap ngoai di qua tat ca cac phan tu   
		for(int j =i+1; j< userCount; j++){               // vong lap trong so sanh tung cap phan tu  
...
void lockUnlockUser() {
    ...
    for(int i =0; i< userCount; i++) {
        if(strcmp(users[i].id, searchID)== 0) {
            ...
            // neu tim thay user tiep tuc so sanh va dung toan tu 3 ngoi de goi y cho admin lua chon   
            char confirm;
            scanf("%c", &confirm);                
            
            if(confirm == 'Y' || confirm =='y') {
              strcpy(users[i].status, strcmp(users[i].status, "open") == 0 ? "locked" : "open");    
              //dung ham strcmp de so sanh tinh trang hien tai voi open neu giong nhau tra ve 0 , sau do dung ham strcpy de cap nhat trang thai moi 
...
int loginAdmin() {  
    ...
    Admin storedAdmin;                                   // khai bao bien co kieu du lieu admin de luu lai du lieu sau khi doc file  
    fread(&storedAdmin, sizeof(Admin), 1, file);         // fread doc file luu vao bien admin  
    fclose(file);
    ...
    while(1){
        ...
        // sdt mac dinh la mat khau  
        printf("Enter password(number phone): ");
        getPassword(inputPassword); 

        // So sánh username và password
        if(strcmp(storedAdmin.username, inputUsername) == 0 && strcmp(storedAdmin.password, inputPassword) == 0) { // dung ham strcmp de so sanh 2 chui  
            printf("Login successful! Welcome, Admin.\n");
            return 1;                                     // tra ve 1 neu dung ten va mat khau  
        }else{
...
// ham chuyen doi ki tu mat khau nhap vao thanh *** 
void getPassword(char* password) {
    char ch;
    int i = 0;
    while(1) {
        ch = getch();                           // ham getch doc 1 ki tu tu ban phim ma khong hien ra mang hinh  
        if(ch == 13) {                         // ch = 13 la ma ASCII cua Enter  
            password[i] = '\0';                  
            break;
        }else if(ch == 8) {                    //Neu nhan  Backspace (ASCII 8), xoa ki tu truoc do  
            if (i > 0) {
                i--;                          
                printf("\b \b");  
            }
        }else if(ch >= 32 && ch <= 126){   // Neu nhap ki tu hop le luu vao mang va hien thi * 
            password[i] = ch;
            i++;
            printf("*");
        }
